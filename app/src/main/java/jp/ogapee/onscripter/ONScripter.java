...
public class ONScripter extends Activity implements AdapterView.OnItemClickListener, Runnable
{
	...
	private void runDownloaderSub()
	{
		...
		if (url.length() == 0){
			...
			mChecker.checkAccess(new LicenseCheckerCallback(){
				...
				public void allow(int reason){
					...
					if (count > 0){
						...
						downloader = new jp.ogapee.onscripter.DataDownloader(zip_dir, zip_filename, gCurrentDirectoryPath, version_filename, url, file_size, handler);
					}
				}
			...
			downloader = new jp.ogapee.onscripter.DataDownloader(zip_dir, zip_filename, gCurrentDirectoryPath, version_filename, url, -1, handler);
		}
	}
	...
	private void runSDLApp() {
		...
		for (int i=0 ; i<6 ; i++) layout2_2.addView(btn[i], i);
	...
	public boolean onPrepareOptionsMenu(Menu menu)
	{
		...
		if (mGLView != null){
			...
			if (gRenderFontOutline)
				sm.add(Menu.NONE, Menu.FIRST+num_alpha+3, 0, getResources().getString(R.string.menu_hide_font_outline));
			else
				sm.add(Menu.NONE, Menu.FIRST+num_alpha+4, 0, getResources().getString(R.string.menu_show_font_outline));
